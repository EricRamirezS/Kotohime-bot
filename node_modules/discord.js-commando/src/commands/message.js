...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(!this.command.hasPermission(this)) {
			...
			return await this.reply(`No tienes permisos para usar el comando \`${this.command.name}\`.`);
		}
		...
		if(throttle && throttle.usages + 1 > this.command.throttling.usages) {
			...
			return await this.reply(
				`No puedes usar el comando \`${this.command.name}\` de nuevo hasta despues de ${remaining.toFixed(1)} segundos.`
			);
		}
		...
		if(!args && this.command.args) {
			...
			if(!args) return await this.reply('Comando cancelado.');
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				return await this.reply(stripIndents`
					Un error a ocurrido mientras se ejecutaba el comando: \`${err.name}: ${err.message}\`
					No deberías estar recibiendo un error como este.
					por favor contacta a ${ownerList}${invite ? ` en este servidor: ${invite}` : '.'}
				`);
			}
		}
	}
