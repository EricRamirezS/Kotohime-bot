...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `El comando \`${this.name}\` solo puede ser usado por el dueño del bot.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `El comando \`${this.name}\` requiere que tengas el permiso: "${permissions[missing[0]]}".`;
				}
				return oneLine`
					El comando \`${this.name}\` requiere que tengas los siguientes permisos:
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly':
				return message.reply(`El comando \`${this.name}\` solo puede usarse en servidores.`);
			case 'nsfw':
				return message.reply(`El comando \`${this.name}\` solo puede usarse en canales NSFW.`);
			case 'permission': {
				if(data.response) return message.reply(data.response);
				return message.reply(`No tienes permisos para usar el comando \`${this.name}\`.`);
			}
			case 'clientPermissions': {
				if(data.missing.length === 1) {
					return message.reply(
						`Necesito el permiso "${permissions[data.missing[0]]}" para que el comando \`${this.name}\` funcione.`
					);
				}
				return message.reply(oneLine`
					Necesito los siguientes permisos para que el comando  \`${this.name}\` funcione:
					${data.missing.map(perm => permissions[perm]).join(', ')}
				`);
			}
			case 'throttling': {
				return message.reply(
					`No puedes usar el comando \`${this.name}\` nuevamente hasta pasados ${data.remaining.toFixed(1)} segundos.`
				);
			}
			default:
				return null;
	...
	onError(err, message, args, fromPattern, result) { // eslint-disable-line no-unused-vars
		...
		const ownerList = owners ? owners.map((usr, i) => {
			const or = i === owners.length - 1 && owners.length > 1 ? 'o ' : '';
			return `${or}${escapeMarkdown(usr.username)}#${usr.discriminator}`;
		}).join(owners.length > 2 ? ', ' : ' ') : '';

		const invite = this.client.options.invite;
		return message.reply(stripIndents`
			Ha ocurrido un error al ejecutar el comando: \`${err.name}: ${err.message}\`
			No deberías recibir un error como este.
			Por favor contacta al  ${ownerList || 'dueño del bot'}${invite ? ` En este servidor: ${invite}` : '.'}
		`);
	}
	...
	static usage(command, prefix = null, user = null) {
		...
		return `${prefixPart || ''}${prefix && user ? ' o ' : ''}${mentionPart || ''}`;
	}
