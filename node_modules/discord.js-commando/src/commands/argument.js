	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				${empty ? this.prompt : valid ? valid : `Has entregado un parametro inválido: ${this.label}. Intentalo nuevamente.`}
				${oneLine`
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(val) {
					...
					prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							Has entregado un parametro inválido: ${this.label},
							"${escaped.length < 1850 ? escaped : '[Muy largo para mostrar]'}".
							Inténtalo nuevamente.
						`}
						${oneLine`
							Responde con \`cancelar\` para cancelar el comando, o \`finalizar\` para detener la entrada en este punto.
							${wait ? `El comando será cancelado automáticamente en ${this.wait} segundos.` : ''}
						`}
					`));
					...
					prompts.push(await msg.reply(stripIndents`
						${this.prompt}
						${oneLine`
							Responde con \`cancel\` para cancelar el comando, o  \`finish\` para detener la entrada.
							${wait ? `El comando será cancelado automáticamente en ${this.wait} segundos, a menos que respondas.` : ''}
						`}
					`));
				}
