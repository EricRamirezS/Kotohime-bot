...
class CommandArgument {
	...
	constructor(command, info) { // eslint-disable-line complexity
		if(!command) throw new Error('El comando debe ser especificado.');
		if(!info) throw new Error('La información de los comandos debe ser especificada.');
		if(typeof info.key !== 'string') throw new TypeError('La key de los comandos debe ser un String.');
		if(info.label && typeof info.label !== 'string') throw new TypeError('La etiqueta de los comandos debe ser un String.');
		if(typeof info.prompt !== 'string') throw new TypeError('El argumento prompt del comando debe ser un String.');
	...
	async obtain(msg, value) {
		...
		while(!valid || typeof valid === 'string') {
			...
			await msg.reply(stripIndents`
				${!value ? this.prompt : valid ? valid : `Entregaste un \`${this.label}\` invalido. Por favor intentalo nuevamente.`}
				${oneLine`
					Responde con \`cancelar\` para cancelar el comando.
					${wait ? `El comando será cancelado automáticamente en ${this.wait} segundos.` : ''}
				`}
			`);
			...
			if(value.toLowerCase() === 'cancelar') return null;
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(value) {
					...
					await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							Entregaste un \`${this.label}\`,
							"${escaped.length < 1850 ? escaped : '[demasiado largo]'}".
							Por favor intentalo nuevamente.
						`}
						${oneLine`
							Responde con \`cancelar\` para cancelar el comando, o \`finalizar\` para terminar la entrada hasta este punto.
							${wait ? `El comando será cancelado automáticamente en ${this.wait} segundos.` : ''}
						`}
					`);
					...
					await msg.reply(stripIndents`
						${this.prompt}
						${oneLine`
							Responde con \`cancelar\` para cancelar el comando, o \`finalizar\` para terminar la entrada.
							${wait ? `El comando será cancelado automáticamente en ${this.wait} segundos.` : ''}
						`}
					`);
				}
				...
				if(lc === 'cancelar') return null;
				if(lc === 'finalizar') return results.length > 0 ? results : null;
				valid = await this.validate(value, msg);
			}
